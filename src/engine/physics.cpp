...
extern const float DASHVELGROUND = 150.0f;
extern const float DASHVELAIR = 50.0f;
    ...
    }else if(pl->physstate == PHYS_FALL && pl->jumping && pl->walljumpCount > 0)
    {
        ...
        for(int x = -1; x < 2; x++)
        {
            ...
            for(int y = -1; y < 2; y++)
            {
                ...
                vec dir = vec(x, y, 0).normalize();
                vec origin(pl->o);
                float dist = raycube(origin, dir, 50, RAY_CLIPMAT|RAY_ALPHAPOLY, 0);
...
ICOMMAND(dash, "D", (int *down), { game::dodash(*down!=0); });
ICOMMAND(slide, "D", (int *down), { game::doslide(*down!=0); });
