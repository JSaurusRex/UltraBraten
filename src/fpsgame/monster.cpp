    ...
    struct monster : fpsent
    {
        ...
        void monsterpain(int damage, fpsent *d)
        {
            ...
            if((health -= damage)<=0)
            {
                ...
                printf("monster died, lmao \n");

                if(d->type==ENT_PLAYER)
                {
                    float dist = d->o.dist(o);
                    if(dist > 100)
                        return;
                    for(int i = 1; i < 12; i++)
                    {
                        d->addammo(i, (rand()%12-i)/3);
                    }
                    printf("giving player ammo");

                    d->health += monstertypes[mtype].health/(dist+1);
                }

                // newEntity(0);

                // printf("amount entities: %i\n", p_ments->length());
                // entity &e = server::sents->add();
                // e.type = 8;
                // e.o = d->o;
                ...
                // for(int i = 0; i < p_ments->length(); i++)
                // {
                //     entity * ent = &(*p_ments)[i];
                //     printf("ent type: %i  attr1 %i attr2 %i attr3 %i attr4 %i attr5 %i\n", ent->type, ent->attr1, ent->attr2, ent->attr3, ent->attr4, ent->attr5);
                //     ent->o.z += 2;
                // }
            }
            else
